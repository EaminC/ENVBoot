#!/usr/bin/env bash
#
# e2e-baremetal.sh - End-to-end test for physical:host leases
#
# Creates a physical host lease, launches via Nova with reservation hint, and provides SSH info.
# Reads config/.env if present, or uses defaults.
#
# Usage:
#   bash scripts/e2e-baremetal.sh
#
#   Override with environment variables:
#   KEY_NAME=mykey IMAGE="Ubuntu 22.04" bash scripts/e2e-baremetal.sh
...
echo "================================================================"
echo "  End-to-End Test: Bare Metal (physical:host)"
echo "================================================================"
...
# Step 1: Create lease
log_info "Step 1/5: Creating physical:host lease..."
...
# Step 3: Launch baremetal (Nova path with reservation hint)
log_info "Step 3/5: Launching baremetal server (Nova + reservation.id)..."
log_info "  This may take 10-15 minutes for baremetal provisioning..."
...
if [ "$SERVER_OK" != "true" ]; then
    log_error "Baremetal launch failed"
    cat /tmp/api6_out.json | jq '.'
    exit 1
fi

log_info "✓ Baremetal server launched successfully"
...
if [ -z "$FLOATING_IP" ]; then
    echo "  Note: Using fixed IP (requires VPN or tenant network access)"
fi
echo ""
echo "================================================================"
echo ""
